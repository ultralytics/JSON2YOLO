...
JSON2YOLO is a small set of standalone scripts that convert third-party annotation JSON into Ultralytics YOLO label files (a `labels/` tree, optional copied `images/`, and a dataset YAML under `--save-dir`). It is superseded by `convert_coco()` in the main `ultralytics` package — the README banner points users there; this repo is maintained but no longer actively extended.
